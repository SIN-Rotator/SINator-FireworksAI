...
echo "Installing SIN-Hermes-Provider-Bundle (Router Mode)..."
echo ""
echo "This installs a local pool-router that auto-failovers between"
echo "sinatorpool1 / sinatorpool2 / sinatorpool3 on 429/412/5xx."
echo ""

# 1. Router Config (localhost:9998)
curl -fsSL "$REPO/config/fireworks-router.yaml" -o "$HERMES_HOME/config.yaml"

# 2. Download pool-router.py
...
# 3. Start pool-router (background, survives terminal close)
...
echo ""
echo "Router Mode configured: localhost:9998 → sinatorpool1/2/3 (auto-failover)"
echo ""
echo "Setup:"
echo "  hermes auth add custom:fireworks --type api-key --api-key \"\$FIREWORKS_AI_API_KEY\""
echo ""
echo "Manage router:"
echo "  pgrep -f pool-router.py    # Check if running"
echo "  pkill -f pool-router.py    # Stop"
echo "  tail -f ~/.hermes/logs/pool-router.log"
echo ""
echo "Done!"
