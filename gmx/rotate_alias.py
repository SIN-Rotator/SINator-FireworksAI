...
async def rotate_alias(name: Optional[str] = None, port: int = DEFAULT_CDP_PORT) -> Dict[str, Any]:
    """Delete the current alias and create a fresh one in a single pass."""
    return await get_service().rotate_alias(new_alias_name=name, cdp_port=port)


def _add_args(p):
    p.add_argument("--name", default=None, help="New alias name without @gmx.de (default: random)")


async def _action(args) -> Dict[str, Any]:
    return await rotate_alias(name=args.name, port=args.port)


if __name__ == "__main__":
    run(_action, description="Rotate (delete + create) the GMX alias", add_args=_add_args)
