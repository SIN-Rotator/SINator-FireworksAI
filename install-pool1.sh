...
echo "Installing SIN-Hermes-Provider-Bundle (Pool 1)..."

# 1. Provider Config
curl -fsSL "$REPO/config/fireworks-pool1.yaml" -o "$HERMES_HOME/config.yaml"
...
if ! grep -q "import _ua_patch" "$HERMES_HOME/hermes-agent/run_agent.py" 2>/dev/null; then
  # Insert after the 'import os' line (line ~45)
...
echo ""
echo "Pool 1 configured: sinatorpool1.delqhi.com"
echo ""
echo "Setup:"
